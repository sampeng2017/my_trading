...
| **Alpaca** | Stock prices | Optional | Yes (free) |
| **Finnhub** | News data | Optional | Yes (60 calls/min) |
...
### 2.3 Get Finnhub API Key (Optional)
...
# Optional: Better price data (falls back to Yahoo Finance)
...
2. **System runs automatically** (if launchd configured) or manually:
   ```bash
   source venv/bin/activate && source .env
   python src/main_orchestrator.py
   ```
3. **Receive alerts** via iMessage/email for trade recommendations
4. **You decide** whether to execute trades (system never trades automatically)
